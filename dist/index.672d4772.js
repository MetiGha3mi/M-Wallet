...
/* -------------Application state------------- */ let currentAccount, nextAccount;
let sorted = false;
let coinObj;
let coinWallet;
let selectedCoin;
...
const btnSubmitCryptoExchange = document.querySelector(".cce-btnexchange-submit");
const btnSendSubmit = document.querySelector(".cce-btnsend-submit");
const btnCCeexCoinItem = [
    ...document.querySelectorAll(".cceex-coin-item")
];
// App Covers
...
const cceexMessageCoinfromname = document.querySelector(".cceex-message-coinfromname");
...
const cceexMessageToCoinName = document.querySelector(".cceex-message-cointoname");
...
/* -------------App Functionalities------------- */ // Displaying account movements
...
const displayCreditNumber = function(acc) {
    const hiddenCredit = acc.creditNumber.slice(-4).padStart(acc.creditNumber.length, "*");
...
const balanceMovementOperation = function(acc) {
    ...
    acc.movements.push(btnExchangeMode.classList.contains("deposit") ? amount : -amount);
};
...
const displayCurrencyAmount = function() {
    cryptoCurrenciesArr.map((crp, i)=>{
        document.querySelector(`.${crp}-amount`).textContent = currentAccount.coinWallet[crp].amount;
    });
};
...
/* -------------App EventListeners------------- */ // Login section //
// App info
...
btnLoginSubmit.addEventListener("click", function(e) {
    ...
    if (inputLoginUsername.value !== "" && inputLoginPassword.value !== "") {
        currentAccount = accounts.find((acc, i)=>acc.username === inputLoginUsername.value || acc.password === +inputLoginPassword.value);
        currentAccount === account1 ? nextAccount = account2 : nextAccount = account1;
        if (currentAccount?.username === inputLoginUsername.value && currentAccount?.password === +inputLoginPassword.value) {
            ...
            setTimeout(function() {
                ...
                welcomeMessage.textContent = `Welcome to your account ${currentAccount.ownerName.split(" ")[0]}`;
...
btnExchangeMode.addEventListener("click", function() {
    ...
    btnExchangeMode.textContent = `${btnExchangeMode.classList.contains("deposit") ? "Deposit" : "Withdrawal"}`;
});
btnSubmitExchange.addEventListener("click", function(e) {
    e.preventDefault();
    if (doOnce === true) {
        if (inputAmountValue.value !== "" && inputPinValue.value !== "") {
            if (+inputAmountValue.value > 0 && +inputPinValue.value === currentAccount.password) {
                if (btnExchangeMode.classList.contains("deposit")) {
                    doExchangeMoney();
                    doOnce = false;
                } else if (currentAccount.totalBalance > +inputAmountValue.value) {
                    doExchangeMoney();
                    doOnce = false;
                } else {
                    warningBalance.textContent = "You can't withdrawal more than you have!";
                    warningBalance.classList.add("warning-appear");
                }
            } else {
...
btnCCeBsMode.addEventListener("click", function() {
    ...
    btnCCeBsMode.textContent = btnCCeBsMode.classList.contains("buy") ? "Buy" : "Sell";
    ccebsResLabel.textContent = btnCCeBsMode.classList.contains("buy") ? "Will earn:" : "Should pay:";
    ccebsWithLabel.textContent = btnCCeBsMode.classList.contains("buy") ? "With:" : "To:";
});
btnCCebsWithSelect.addEventListener("click", function() {
    ...
    currencyToBuy = btnUSDTSide.classList.contains("selected-side") ? `USDT` : `USD`;
...
btnCCeBsSubmit.addEventListener("click", function(e) {
    ...
    const exchangeCrypto = function(amount, willEarn) {
        willEarn = selectedCoin === "tether" ? amount * 1 : amount * cryptoCurrenciesObj[selectedCoin].exchangements.oneUSDorUSDT;
            ...
            if (btnCCeBsMode.classList.contains("buy")) {
                ...
                if (currentAccount.coinWallet[selectedCoin].amount >= amount) {
                    ...
                    if (currencyToBuy === "USDT" && selectedCoin !== "tether") {
                        currentAccount.coinWallet.tether.amount += amount * +cryptoCurrenciesObj[selectedCoin].price.replace(",", "");
                        inputCCBuySellResult.value = `${amount * +cryptoCurrenciesObj[selectedCoin].price.replace(",", "")} $`;
                        doBuySellCrypto();
                    } else if (currencyToBuy === "USD") {
                        currentAccount.movements.push(amount * +cryptoCurrenciesObj[selectedCoin].price.replace(",", ""));
                        inputCCBuySellResult.value = `${amount * +cryptoCurrenciesObj[selectedCoin].price.replace(",", "")} $`;
                        doBuySellCrypto();
                    } else {
...
btnSubmitCryptoExchange.addEventListener("click", function(e) {
    ...
    let selectedToCoin = selectedCoin.charAt(0).toUpperCase() + selectedCoin.slice(1);
    let amount = inputAmountCryptoExchange.value;
    if (doOnce === true) {
        if (inputAmountCryptoExchange.value !== "" && +inputAmountCryptoExchange.value !== 0) {
            if (toCoinName !== "---") {
                if (selectedCoin !== toCoinName) {
                    if (currentAccount.coinWallet[selectedCoin].amount >= amount) {
                        currentAccount.coinWallet[selectedCoin].amount -= +amount;
                        currentAccount.coinWallet[toCoinName].amount += +amount * cryptoCurrenciesObj[toCoinName].exchangements[`one${selectedToCoin === "Tether" ? "USDorUSDT" : selectedToCoin}`];
                        doOnce = false;
